...
INPUT="$1"
if [[ -z "$INPUT" ]]; then
  echo "Usage: pixi run debug <number|padded_number|directory_name>"
  echo "Examples: pixi run debug 1, pixi run debug 001, pixi run debug 001_variables"
  exit 1
fi
